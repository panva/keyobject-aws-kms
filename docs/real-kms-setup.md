...
Re-run this bootstrap with the same repository arguments before the first
tagged release if the roles were created before the release workflow existed.
That updates the live admin-role trust policy to include `refs/tags/v*`; changing
the checked-in policy generator alone does not update IAM.
...
the configured branch subject, the repository's `pull_request` subject, and
release-tag subjects under `refs/tags/v*`. A pull-request subject cannot be
narrowed by branch in AWS, and IAM glob syntax cannot express the release
workflow's numeric `vX.Y.Z` tag filter. Repository workflow controls, tag-push
authority, and the same-repository/maintainer-label gates are therefore part of
the security boundary. Use approval-protected GitHub Environments with
corresponding environment subjects if that boundary is not acceptable.
