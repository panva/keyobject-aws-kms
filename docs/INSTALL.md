...
The package requires Node.js `>=26.7.0`. Its `crypto.createPrivateKey()` accepts
a `URL` and dispatches it through OpenSSL STORE. `isSupported()` and the `check`
command probe the capability so a custom build that omits it fails clearly.
...
The binary distribution supports exactly five targets:

| Target | Status |
| --- | --- |
| `darwin-arm64` | supported, macOS 13.5 or newer |
| `linux-arm64` | supported, glibc 2.28 or newer |
| `linux-x64` | supported, glibc 2.28 or newer |
...
Versioned GitHub release archives are a secondary distribution for the same
five targets. Select `awskms-<version>-<target>.tar.gz`, then verify its
published checksum and attestation before extraction. The archive contains one
matching top-level directory with the provider module, `awskms.cnf`,
`check.mjs`, this guide, the project license, the component manifest, and all
third-party notices and license texts.

Download the archive and the release checksum list, verify only the selected
entry, and then verify its GitHub build-provenance attestation. For example:

```console
gh release download v1.2.3 \
  --repo panva/keyobject-aws-kms \
  --pattern awskms-1.2.3-linux-x64.tar.gz \
  --pattern SHA256SUMS
grep '  awskms-1.2.3-linux-x64.tar.gz$' SHA256SUMS | shasum -a 256 --check
gh attestation verify awskms-1.2.3-linux-x64.tar.gz \
  --repo panva/keyobject-aws-kms
```

`SHA256SUMS` covers all five native archives and all six npm tarballs attached
to that release. The attestation binds the selected bytes to this repository's
release workflow; it does not replace review of the project, its dependencies,
or the release tag.
...
When Node's permission model is enabled, loading any `aws-kms:` URL needs
`--allow-openssl-store`. Startup registration also loads a native addon and
reads the installed packages, so it additionally needs `--allow-addons` and
appropriate filesystem-read permission. A restricted deployment can use the
config route to avoid granting addon loading.
