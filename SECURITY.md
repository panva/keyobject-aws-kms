...
[Security page](https://github.com/panva/keyobject-aws-kms/security) to report a
vulnerability privately when GitHub presents that option. If it is unavailable,
contact the [repository owner](https://github.com/panva) to arrange a private
channel before sending technical details. Do not open a public issue for an
unpatched vulnerability. Include the affected platform and Node/OpenSSL
versions, a minimal reproducer, expected impact, and whether the issue requires
control of local files, environment variables, an `aws-kms:` URI, AWS
credentials, or a KMS key policy.
...
Security fixes target the latest published package. Keep the core package and
its native platform package on the same exact version; the runtime rejects
version skew.
