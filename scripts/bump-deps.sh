...
if [[ $cur != "$latest" ]]; then
  ...
  # The body is not decoration. A bump moves vendored s2n, and
  # scripts/patch-s2n-openssl4.sh asserts an exact ASN1_STRING_data call-site
  # count -- so it will FAIL the build if upstream fixed B5 or moved the code.
...
build here is expected to mean one of:
  ...
  * upstream fixed the OpenSSL 4.0 issue (B5) -- delete the patch and its
...
# Exit 1 with nothing to do, so the caller can skip the push/PR without parsing
# stdout. Not an error condition -- the caller tests for it explicitly.
(( bumped > 0 )) || exit 1
