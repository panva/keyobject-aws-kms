...
removed_macos_target='darwin-''x64'
removed_macos_runner='macos-15-''intel'
removed_macos_job='macos-''x64'
escaped_removed_oid=${removed_oid//./\\.}
stale_marker='(^|[^[:alnum:]_])(todo|wip|journal|work[ -]?in[ -]?progress)([^[:alnum:]_]|$)'
forbidden_source="$removed_curve|$removed_enum|$escaped_removed_oid|$removed_macos_target|$removed_macos_runner|$removed_macos_job|/Users/[^/]+/|/home/[^/]+/|/private/var/folders/"
...
case ${1:-} in
  source)
    ...
    echo 'ok: no stale files or markers, private workspace paths, or removed-feature residue'
    ;;
