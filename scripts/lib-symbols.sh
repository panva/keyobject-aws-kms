...
# and none of which it needs to. Counting them as requirements would reject a
# module that correctly tests their absence at runtime.
...
# bare `CRYPTO_free`, so an exact-match comparison would report exported core
# symbols as missing. Normalize symbol versions for both executables and shared
# libraries.
