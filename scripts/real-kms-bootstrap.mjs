 ...
 * admin role be assumed from that repository's main branch, pull requests, and
 * stable release tags, so CI needs no long-lived secrets.
 *
  ...
  Statement: [
    {
       ...
       * material carrying exactly the test ownership tag set. */
      ...
      Condition: {
        StringEquals: {
          ...
          'aws:RequestTag/awskms-provider-test': '1',
        },
        Bool: { 'kms:MultiRegion': 'false' },
        'ForAllValues:StringEquals': {
          'aws:TagKeys': [
            'awskms-provider-test',
            'awskms-run',
            'awskms-role',
            'awskms-spec',
          ],
        ...
        Null: {
          'aws:RequestTag/awskms-run': 'false',
          'aws:RequestTag/awskms-role': 'false',
          'aws:RequestTag/awskms-spec': 'false',
        },
      },
    },
    {
    ...
    },
    {
      ...
      Condition: {
        StringEquals: { 'aws:RequestTag/awskms-provider-test': '1' },
        'ForAllValues:StringEquals': {
          'aws:TagKeys': [
            'awskms-provider-test',
            'awskms-run',
    ...
    {
      Sid: 'ReadOwnershipTags',
      Effect: 'Allow',
      Action: 'kms:ListResourceTags',
      Resource: `arn:aws:kms:*:${acct}:key/*`,
    },
    {
      /* CreateAlias is authorised against both the alias and target key. */
      Sid: 'CreateOurAliasNames',
      Effect: 'Allow',
      Action: 'kms:CreateAlias',
      Resource: `arn:aws:kms:*:${acct}:alias/awskms-*`,
    },
    {
      Sid: 'CreateAliasesOnlyForOwnedKeys',
      Effect: 'Allow',
      Action: 'kms:CreateAlias',
      Resource: `arn:aws:kms:*:${acct}:key/*`,
      Condition: {
 ...
 *
 * A wildcard after the repository name cannot absorb the immutable `@<id>`
 * suffix on the owner. GitHub reports the exact prefix, so use it verbatim
 * without a wildcard.
 *
 ...
 *
 * IAM cannot express the release workflow's numeric vX.Y.Z glob, so the tag
 * subject is necessarily v*. The release workflow itself accepts only stable
 * numeric tags and rechecks the remote tag before any publication. Repository
 * tag-push authority is therefore also part of this trust boundary.
 */
const adminTrust = (acct, callerArn, repo, ref) => {
  ...
  if (repo) {
    const prefix = githubSubPrefix(repo);
    st.push({
      Effect: 'Allow',
  ...
  if (exists) {
    ...
    await withPrincipalRetry(
      () => awsTry(['iam', 'create-role', '--role-name', name,
        '--description', 'keyobject-aws-kms test harness',
