...
#
#     `check` has no caller in CI: scripts/bump-deps.sh resolves the latest
#     release itself, because it must do so for the cmake-pinned dependencies
#     too, which are not in this manifest. Kept because it is the quickest way to
#     ask the question by hand, and exit 2 still distinguishes drift from
#     breakage.
...
#
# Needs `gh`, which shells out to the official tool rather than reimplementing
# GitHub's API -- the same
...
# A run that checked NOTHING must not report success. Every mode above iterates
# the manifest, so an empty, moved or unreadable manifest makes the loop body
# never execute -- `fail` stays 0 and this prints "all checks passed" having
# verified nothing at all. Same vacuous pass check-symbol-floor.sh had.
rows=$(grep -v '^[[:space:]]*#' third_party/vendored.manifest 2>/dev/null | grep -c '|' || true)
if (( rows == 0 )); then
  echo "FAILED: third_party/vendored.manifest yielded no entries -- nothing was verified" >&2
  exit 1
fi
echo "all checks passed ($rows vendored dependency(ies))"
