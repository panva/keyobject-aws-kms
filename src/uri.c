...
int awskms_uri_parse(const char *uri, AWSKMS_URI *out,
...

  {
    ...
    if (search.length > 0) {
      ...
      /* Reject unknown and duplicate query attributes for the same reason as
       * path ones. URLSearchParams::get() returns only the first value, so
       * checking duplicates before taking either value is essential: silently
       * choosing one lets different consumers interpret the same URI as naming
       * different credentials or endpoints. */
      {
        ada_url_search_params_keys_iter it = ada_search_params_get_keys(params);
        unsigned int profiles = 0, endpoints = 0;
        while (it != NULL && ada_search_params_keys_iter_has_next(it)) {
          ...
          if (k.length == 7 && memcmp(k.data, "profile", 7) == 0) {
            name = "profile";
            count = &profiles;
          } else if (k.length == 8 && memcmp(k.data, "endpoint", 8) == 0) {
            name = "endpoint";
            count = &endpoints;
          } else {
            AWSKMS_raise(handle, AWSKMS_R_INVALID_URI,
          ...
          if (++*count > 1) {
            AWSKMS_raise(handle, AWSKMS_R_INVALID_URI,
                         "query attribute \"%s\" given more than once", name);
            ada_free_search_params_keys_iter(it);
            goto out;
          }
        }
