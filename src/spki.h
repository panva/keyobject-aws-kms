 ...
 * resulting default-provider EVP_PKEY then serves three purposes at once, which
 * is why there is no hand-rolled DER anywhere in this codebase:
 *
...
typedef struct awskms_pubkey_st {
  /* Default-provider public key parsed from the SPKI. Owned. */
  EVP_PKEY *pkey;
