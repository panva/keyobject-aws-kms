...
static int compute_tr(AWSKMS_PUBKEY *p, OSSL_LIB_CTX *libctx, int fips_required,
                      const OSSL_CORE_HANDLE *handle) {
  EVP_MD *shake = NULL;
  EVP_MD_CTX *ctx = NULL;
  int ok = 0;

  if ((shake = EVP_MD_fetch(libctx, "SHAKE256",
                            awskms_dependency_propq(fips_required))) == NULL) {
...
static int expected_rsa_bits(const AWSKMS_KEYSPEC *spec) {
  if (strcmp(spec->kms_key_spec, "RSA_2048") == 0) return 2048;
  if (strcmp(spec->kms_key_spec, "RSA_3072") == 0) return 3072;
  if (strcmp(spec->kms_key_spec, "RSA_4096") == 0) return 4096;
  return 0;
}
...
int awskms_pubkey_from_spki(AWSKMS_PUBKEY *out, const unsigned char *der,
  ...
  if (der_len > (size_t)LONG_MAX) {
    AWSKMS_raise(handle, AWSKMS_R_MALFORMED_PUBLIC_KEY,
                 "public key is too large to decode: %zu bytes", der_len);
    goto err;
  }

  if ((out->der = OPENSSL_memdup(der, der_len)) == NULL) goto err;
  out->der_len = der_len;
  p = out->der;
   ...
   * simply fail. */
  out->pkey = d2i_PUBKEY_ex(NULL, &p, (long)out->der_len, libctx,
                            awskms_dependency_propq(fips_required));
  if (out->pkey == NULL) {
    AWSKMS_raise(handle, AWSKMS_R_MALFORMED_PUBLIC_KEY,
  ...
  if (p != out->der + out->der_len) {
    AWSKMS_raise(handle, AWSKMS_R_MALFORMED_PUBLIC_KEY,
                 "%s SubjectPublicKeyInfo has %zu trailing byte(s)",
                 out->spec->kms_key_spec,
                 (size_t)(out->der + out->der_len - p));
    goto err;
  }
  ...
  switch (out->spec->family) {
    ...
    case AWSKMS_FAMILY_ML_DSA:
      if (!load_raw_public(out, handle)) goto err;
      if (!compute_tr(out, libctx, fips_required, handle)) goto err;
      break;
    case AWSKMS_FAMILY_RSA: {
      int actual = EVP_PKEY_get_bits(out->pkey);
      int expected = expected_rsa_bits(out->spec);

      if (expected == 0 || actual != expected) {
        AWSKMS_raise(handle, AWSKMS_R_MALFORMED_PUBLIC_KEY,
                     "%s public key has %d bits, expected %d",
                     out->spec->kms_key_spec, actual, expected);
        goto err;
      }
      break;
    }
...
int awskms_pubkey_dup(AWSKMS_PUBKEY *out, const AWSKMS_PUBKEY *src) {
  if (out == NULL || src == NULL || out == src) return 0;
  if ((src->der == NULL) != (src->der_len == 0) ||
      (src->raw == NULL) != (src->raw_len == 0))
    return 0;

  out->spec = src->spec;
  if (src->der_len != 0) {
    if ((out->der = OPENSSL_memdup(src->der, src->der_len)) == NULL) goto err;
    out->der_len = src->der_len;
  }
  if (src->pkey != NULL) {
    if (EVP_PKEY_up_ref(src->pkey) != 1) goto err;
    out->pkey = src->pkey;
  }
  if (src->raw_len != 0) {
    if ((out->raw = OPENSSL_memdup(src->raw, src->raw_len)) == NULL) goto err;
    out->raw_len = src->raw_len;
  }
  memcpy(out->tr, src->tr, sizeof(out->tr));
  out->have_tr = src->have_tr;
  return 1;
...
int awskms_mu(OSSL_LIB_CTX *libctx, int fips_required,
  ...
  shake =
      EVP_MD_fetch(libctx, "SHAKE256", awskms_dependency_propq(fips_required));
  if (shake == NULL) {
    AWSKMS_raise(handle, AWSKMS_R_INTERNAL_ERROR,
                 "SHAKE256 is unavailable, so ML-DSA mu cannot be computed");
    goto out;
