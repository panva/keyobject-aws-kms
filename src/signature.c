...
static int set_digest(AWSKMS_SIG_CTX *ctx, const char *mdname) {
  ...
  md = EVP_MD_fetch(ctx->provctx->libctx, mdname,
                    ctx->propq != NULL ? ctx->propq : AWSKMS_EXCLUDE_SELF);
  if (md == NULL) {
...
static int awskms_sig_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_DIGEST)) !=
    ...
    char name[64] = {0};
    char *namep = name;
    if (!OSSL_PARAM_get_utf8_string(p, &namep, sizeof(name))) return 0;
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_INSTANCE)) !=
      NULL) {
    char *instp = ctx->instance;

    if (p->data_type != OSSL_PARAM_UTF8_STRING) return 0;
    if (!OSSL_PARAM_get_utf8_string(p, &instp, sizeof(ctx->instance))) {
      AWSKMS_raise(ctx->provctx->handle, AWSKMS_R_UNSUPPORTED_PARAMETER,
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_MGF1_DIGEST)) !=
      NULL) {
    if (p->data_type != OSSL_PARAM_UTF8_STRING || p->data == NULL) return 0;
    if (ctx->md == NULL || EVP_MD_is_a(ctx->md, p->data) != 1) {
      AWSKMS_raise(ctx->provctx->handle, AWSKMS_R_UNSUPPORTED_PARAMETER,
...
static size_t max_sig_size(const AWSKMS_SIG_CTX *ctx) {
  int size = EVP_PKEY_get_size(ctx->key->pub.pkey);
...
static int sign_digest(AWSKMS_SIG_CTX *ctx, unsigned char *sig, size_t *siglen,
  ...
  return awskms_kms_sign(ctx->provctx, &ctx->key->uri, algorithm, "DIGEST",
...
static int awskms_sig_verify(void *vctx, const unsigned char *sig,
  ...
  pctx = EVP_PKEY_CTX_new_from_pkey(ctx->provctx->libctx, ctx->key->pub.pkey,
                                    AWSKMS_EXCLUDE_SELF);
...
static int compute_mu(AWSKMS_SIG_CTX *ctx, const unsigned char *msg,
  ...
  if (!ctx->key->pub.have_tr) {
    AWSKMS_raise(ctx->provctx->handle, AWSKMS_R_INTERNAL_ERROR,
                 "no cached tr for %s",
                 awskms_key_spec(ctx->key)->kms_key_spec);
    return 0;
  }
  return awskms_mu(ctx->provctx->libctx, ctx->propq, ctx->key->pub.tr,
...
static int awskms_sig_oneshot_sign(void *vctx, unsigned char *sig,
  ...
  if (sig == NULL) {
    *siglen = spec->sig_len != 0 ? spec->sig_len : max_sig_size(ctx);
    return *siglen != 0;
  }
  ...
  switch (spec->msg_type) {
    case AWSKMS_MSG_RAW:
      ...
      return awskms_kms_sign(ctx->provctx, &ctx->key->uri, algorithm, "RAW",
                             tbs, tbslen, sig, sigsize, siglen);

    case AWSKMS_MSG_EXTERNAL_MU:
      if (!compute_mu(ctx, tbs, tbslen, mu)) return 0;
      return awskms_kms_sign(ctx->provctx, &ctx->key->uri, algorithm,
                             "EXTERNAL_MU", mu, sizeof(mu), sig, sigsize,
                             siglen);
...
static int awskms_sig_oneshot_verify(void *vctx, const unsigned char *sig,
...

  {
    ...
    if (EVP_DigestVerifyInit_ex(mdctx, NULL, NULL, ctx->provctx->libctx,
                                AWSKMS_EXCLUDE_SELF, ctx->key->pub.pkey,
                                n > 0 ? params : NULL) != 1)
      goto out;
  }
