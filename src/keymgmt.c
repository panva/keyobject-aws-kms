...
static int has_public_key(const AWSKMS_KEY *key) {
  size_t len = 0;

  if (key->pub.pkey == NULL || awskms_key_spec(key) == NULL) return 0;
  if (awskms_key_spec(key)->family == AWSKMS_FAMILY_RSA) {
    BIGNUM *n = NULL;
    int ok =
        EVP_PKEY_get_bn_param(key->pub.pkey, OSSL_PKEY_PARAM_RSA_N, &n) == 1;

    BN_free(n);
    return ok;
  }
  return EVP_PKEY_get_octet_string_param(key->pub.pkey, OSSL_PKEY_PARAM_PUB_KEY,
                                         NULL, 0, &len) == 1 &&
         len != 0;
}

static int has_domain_parameters(const AWSKMS_KEY *key) {
  char group[80];
  size_t len = 0;

  if (awskms_key_spec(key)->family != AWSKMS_FAMILY_EC) return 1;
  return key->pub.pkey != NULL &&
         EVP_PKEY_get_utf8_string_param(key->pub.pkey,
...
static int awskms_keymgmt_has(const void *keydata, int selection) {
  ...
  if (key == NULL || awskms_key_spec(key) == NULL) return 0;
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0 && !has_public_key(key))
    return 0;
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
      (key->uri.key_id == NULL || key->uri.key_id[0] == '\0'))
    return 0;
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0 &&
      !has_domain_parameters(key))
    return 0;
  /* OTHER_PARAMETERS, and domain parameters on non-EC types, are irrelevant
   * selections rather than missing data. */
  return 1;
}
...
static int awskms_keymgmt_match(const void *keydata1, const void *keydata2,
  ...
  if (!awskms_keymgmt_has(a, selection) || !awskms_keymgmt_has(b, selection))
    return 0;
  if (a == b) return 1;

  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0 &&
      (awskms_key_spec(a)->family == AWSKMS_FAMILY_EC ||
       awskms_key_spec(b)->family == AWSKMS_FAMILY_EC)) {
    if (awskms_key_spec(a)->family != AWSKMS_FAMILY_EC ||
        awskms_key_spec(b)->family != AWSKMS_FAMILY_EC ||
        EVP_PKEY_parameters_eq(a->pub.pkey, b->pub.pkey) != 1)
      return 0;
  }

  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    /* A KMS public key uniquely identifies its opaque private half. Comparing
     * key ids is weaker: aliases can be repointed and two aliases can name the
     * same key. */
    if (EVP_PKEY_eq(a->pub.pkey, b->pub.pkey) != 1) return 0;
  }
  return 1;
}
...
static int awskms_keymgmt_validate(const void *keydata, int selection,
  ...
  if (checktype != OSSL_KEYMGMT_VALIDATE_FULL_CHECK &&
    ...
    return 0;
  if (!awskms_keymgmt_has(key, selection)) return 0;

  /* The private half is remote and non-exportable, so local pairwise validation
   * is impossible without performing a billable signature. GetPublicKey binds
   * it to this already-validated public half; private-only validation therefore
   * reduces to the presence check above. */
  if ((selection & (OSSL_KEYMGMT_SELECT_PUBLIC_KEY |
                    OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS)) == 0 ||
      ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) == 0 &&
       awskms_key_spec(key)->family != AWSKMS_FAMILY_EC))
    return 1;

  pctx = EVP_PKEY_CTX_new_from_pkey(
      key->provctx != NULL ? key->provctx->libctx : NULL, key->pub.pkey,
      awskms_dependency_propq(key->fips_required));
  if (pctx == NULL) return 0;

  ok = 1;
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0 &&
      awskms_key_spec(key)->family == AWSKMS_FAMILY_EC) {
    ok = checktype == OSSL_KEYMGMT_VALIDATE_QUICK_CHECK
             ? EVP_PKEY_param_check_quick(pctx) == 1
             : EVP_PKEY_param_check(pctx) == 1;
  }
  if (ok && (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
    ok = checktype == OSSL_KEYMGMT_VALIDATE_QUICK_CHECK
             ? EVP_PKEY_public_check_quick(pctx) == 1
...
static AWSKMS_KEY *new_subset_key(const AWSKMS_KEY *src) {
  AWSKMS_KEY *dst = awskms_key_new(src->provctx);

  if (dst == NULL) return NULL;
  dst->pub.spec = src->pub.spec;
  dst->fips_required = src->fips_required;
  return dst;
}

static AWSKMS_KEY *dup_public_key(const AWSKMS_KEY *src) {
  AWSKMS_KEY *dst = awskms_key_new(src->provctx);

  if (dst == NULL) return NULL;
  dst->fips_required = src->fips_required;
  if (src->pub.pkey == NULL || !awskms_pubkey_dup(&dst->pub, &src->pub))
    goto err;
  return dst;
...
static AWSKMS_KEY *dup_domain_parameters(const AWSKMS_KEY *src) {
  AWSKMS_KEY *dst = NULL;
  EVP_PKEY_CTX *pctx = NULL;
  OSSL_PARAM *params = NULL;
  EVP_PKEY *pkey = NULL;

  if (EVP_PKEY_todata(src->pub.pkey, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
                      &params) != 1)
    goto out;
  pctx = EVP_PKEY_CTX_new_from_name(
      src->provctx != NULL ? src->provctx->libctx : NULL,
      src->pub.spec->keytype, awskms_dependency_propq(src->fips_required));
  if (pctx == NULL || EVP_PKEY_fromdata_init(pctx) != 1 ||
      EVP_PKEY_fromdata(pctx, &pkey, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
                        params) != 1)
    goto out;
  if ((dst = new_subset_key(src)) == NULL) goto out;
  dst->pub.pkey = pkey;
  pkey = NULL;
...
static void *awskms_keymgmt_dup(const void *keydata, int selection) {
  ...
  if (!awskms_keymgmt_has(key, selection)) return NULL;

  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    /* The current representation cannot hold a private KMS reference without
     * its public half. Do not pretend a private-only duplicate is exact. */
    if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) == 0) return NULL;
    if (!awskms_key_up_ref((AWSKMS_KEY *)key)) return NULL;
    return (AWSKMS_KEY *)key;
  }
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0)
    return dup_public_key(key);
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0 &&
      awskms_key_spec(key)->family == AWSKMS_FAMILY_EC)
    return dup_domain_parameters(key);
  return new_subset_key(key);
}
