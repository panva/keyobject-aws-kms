...
#
# AWS SDK subprojects can propagate either a static libcrypto archive or a
# dynamic OpenSSL::Crypto dependency to consumers. Either way this module would
# end up with a second libcrypto: a different
...
#
# Every subproject must therefore find the real libcrypto so its configure-time
# probes are accurate and it compiles against the correct headers. After
# add_subdirectory(), libcrypto is stripped only from the
# INTERFACE_LINK_LIBRARIES of the SDK targets.
#
  ...
  foreach(_t IN LISTS _targets)
    ...
    foreach(_lib IN LISTS _libs)
      ...
      if(_bare IN_LIST _AWSKMS_LIBCRYPTO_NAMES
         OR _bare MATCHES "/lib(crypto|ssl)(\\.[0-9.]+)?\\.(so|dylib)(\\.[0-9.]+)?$"
         OR _bare MATCHES "/lib(crypto|ssl)\\.a$"
...
# same fatal outcome. The check covers both and runs on every build because the
