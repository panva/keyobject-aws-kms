...
# `-O3` had inlined, and neither is an OpenSSL symbol (libcrypto exports
# EVP_PKEY_free, not EVP_PKEY_free_pointer). That false positive failed the build
# for a legitimate binary.
#
...
# defined inside the module. A libcrypto linked *dynamically* defines nothing
# here and sails straight past it -- which is exactly what happened on Linux,
# where this guard passed on a module carrying `NEEDED libcrypto.so.3`. Same
# fatal outcome, different mechanism, so both are checked.
if(APPLE)
...
if(_dumper)
  ...
  # Match the library file name, not any path component: a build directory can
  # legitimately be called something like /home/me/libssl-work/.
  string(REGEX MATCHALL "lib(crypto|ssl)\\.(so|dylib)[^ \t\r\n]*" _dyn "${_deps}")
  if(_dyn)
