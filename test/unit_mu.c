...
static void compare(const char *alg, const unsigned char *ctxs, size_t ctxs_len,
  ...
  CHECK(awskms_mu(NULL, 0, tr, ctxs, ctxs_len, msg, msg_len, ours, NULL) == 1,
...
void test_mu(void) {
  ...
  /* Over-long contexts must be refused rather than truncated. */
  {
    ...
    CHECK(awskms_mu(NULL, 0, tr, ctx_max, 256, (const unsigned char *)"x", 1,
                    out, NULL) == 0,
          "a 256-byte context string must be rejected");
    CHECK(awskms_mu(NULL, 0, tr, ctx_max, 255, (const unsigned char *)"x", 1,
                    out, NULL) == 1,
