...
const CASES = [
  ...
  },
  {
    ...
    uri: `awskms:key-id=${realArn('test', 'RSA_4096') ?? 'arn:aws:kms:eu-central-1:111122223333:key/RSA_4096'}`,
    keyType: 'rsa',
  ...
  {
    label: 'ECC_SECG_P256K1',
    uri: uri('test', 'ECC_SECG_P256K1'),
    keyType: 'ec',
    details: { namedCurve: 'secp256k1' },
  },
  {
...
for (const c of CASES) {
  describe(c.label, { skip: skipFor(c.label, c.needs === 'ml-dsa' && !hasMlDsa ? mlDsaSkipReason : false) }, () => {
    ...
    test('equals() identifies the same key across loads', () => {
      ...
      // A different key id is a different key even when the spec matches, so
      // this must be false for every case -- equality compares the public halves,
      // not the URI.
    ...
    test('loads with and without an explicit property query', () => {
      ...
      // path.
      for (const properties of ['provider=awskms', '']) {
...
describe('URI handling', () => {
  test('rejects a URI with no key-id', () => {
    assert.throws(() => createPrivateKey({ key: new URL('awskms:region=eu-central-1') }), {
  ...
  test('rejects an unknown attribute rather than ignoring it', () => {
    assert.throws(
      () => createPrivateKey({ key: new URL('awskms:key-id=alias/test-RSA_2048;object=x') }),
      { code: 'ERR_OSSL_AWSKMS_INVALID_URI' },
    );
  });
  ...
  test('rejects a region that contradicts the key ARN, without a network call', () => {
    assert.throws(
      () =>
        createPrivateKey({
          key: new URL(
            'awskms:key-id=arn:aws:kms:eu-central-1:1:key/RSA_2048;region=us-east-1',
          ),
        }),
  ...
  test('reports an unknown key with a readable code', () => {
    assert.throws(() => createPrivateKey({ key: new URL('awskms:key-id=alias/nothing') }), {
  ...
  test('a URL is only accepted for private keys', () => {
    ...
    assert.throws(() => createPublicKey(new URL('awskms:key-id=alias/test-RSA_2048')), {
  ...
  test('a plain object that merely looks like a URL is not one', () => {
    ...
    // their choosing.
    const spoofed = { href: 'awskms:key-id=alias/test-RSA_2048', protocol: 'awskms:' };
...
describe('coexistence with the default provider', () => {
  ...
  // unaffected. `default_properties = ?provider!=awskms` in the config is what
