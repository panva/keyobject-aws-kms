...
void test_uri(void) {
  ...
  ok("awskms:key-id=alias/my-signer", "alias/my-signer", NULL, NULL, NULL);
  ok("awskms:key-id=alias/my-signer;region=eu-central-1", "alias/my-signer",
     "eu-central-1", NULL, NULL);
  ok("awskms:key-id=1234abcd-12ab-34cd-56ef-1234567890ab;region=us-east-1",
     "1234abcd-12ab-34cd-56ef-1234567890ab", "us-east-1", NULL, NULL);
  ok("awskms:key-id=alias/s;region=eu-central-1?profile=prod", "alias/s",
     "eu-central-1", "prod", NULL);
  ok("awskms:key-id=alias/s?profile=p&endpoint=http://127.0.0.1:4566",
     "alias/s", NULL, "p", "http://127.0.0.1:4566");

  /* --- region inferred from an ARN, which the AWS SDKs do not do ------- */
  ok("awskms:key-id=arn:aws:kms:eu-central-1:111122223333:key/"
     "1234abcd-12ab-34cd-56ef-1234567890ab",
     "arn:aws:kms:eu-central-1:111122223333:key/"
     "1234abcd-12ab-34cd-56ef-1234567890ab",
     "eu-central-1", NULL, NULL);
  ok("awskms:key-id=arn:aws:kms:us-west-2:111122223333:alias/my-signer",
  ...
  ok("awskms:key-id=arn:aws-us-gov:kms:us-gov-west-1:111122223333:key/abcd",
     "arn:aws-us-gov:kms:us-gov-west-1:111122223333:key/abcd", "us-gov-west-1",
     NULL, NULL);
  /* An explicit region that agrees with the ARN is fine. */
  ok("awskms:key-id=arn:aws:kms:eu-central-1:1:key/a;region=eu-central-1",
     "arn:aws:kms:eu-central-1:1:key/a", "eu-central-1", NULL, NULL);
  /* A multi-Region key id is not an ARN and infers nothing. */
  ok("awskms:key-id=mrk-1234abcd12ab34cd56ef1234567890ab;region=eu-west-1",
     "mrk-1234abcd12ab34cd56ef1234567890ab", "eu-west-1", NULL, NULL);
  /* An ARN for some other service must not be mined for a region. */
  ok("awskms:key-id=arn:aws:iam:eu-central-1:111122223333:role/r",
     "arn:aws:iam:eu-central-1:111122223333:role/r", NULL, NULL, NULL);

  /* --- percent decoding ------------------------------------------------ */
  ok("awskms:key-id=alias/with%20space", "alias/with space", NULL, NULL, NULL);
  ok("awskms:key-id=alias/a%3Bb", "alias/a;b", NULL, NULL, NULL);
  ok("awskms:key-id=alias/a%3Db", "alias/a=b", NULL, NULL, NULL);

  /* --- attribute order and separators --------------------------------- */
  ok("awskms:region=eu-central-1;key-id=alias/s", "alias/s", "eu-central-1",
     NULL, NULL);
  ok("awskms:key-id=alias/s;", "alias/s", NULL, NULL, NULL);
  ok("awskms:key-id=alias/s;;region=eu-west-1", "alias/s", "eu-west-1", NULL,
     NULL);

  /* --- rejections ------------------------------------------------------ */
  rejected("awskms:", "no attributes at all");
  rejected("awskms:region=eu-central-1", "no key-id");
  rejected("awskms:key-id=", "empty key-id");
  rejected("awskms:key-id=a;key-id=b", "repeated key-id");
  rejected("awskms:key-id=a;object=b", "unknown path attribute");
  rejected("awskms:key-id=a;type=private", "a pkcs11 attribute, not ours");
  rejected("awskms:key-id=a?region=eu-central-1",
           "region belongs in the path, not the query");
  rejected("awskms:key-id=a?nope=1", "unknown query attribute");
  rejected("awskms:key-id=a;region", "path attribute without a value");
  rejected("awskms:key-id=a#frag", "fragment");
  rejected("awskms:key-id=alias/%zz", "malformed percent escape");
  rejected("awskms:key-id=alias/%2", "truncated percent escape");
  rejected("awskms:key-id=alias/a%00b", "percent-encoded NUL");
  rejected("pkcs11:object=a", "wrong scheme");
  rejected("awskms:key-id=arn:aws:kms:eu-central-1:1:key/a;region=us-east-1",
