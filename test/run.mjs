...
const modulePath = resolve(
  process.env.AWSKMS_MODULE ??
    join(root, 'build', process.platform === 'darwin' ? 'awskms.dylib' : 'awskms.so'),
);
...
if (usesStub) {
   ...
   * can wipe an entry mid-assertion -- a flake whose appearance depends purely on
   * scheduling, and which duly showed up only after switching node binaries.
   *
  ...
  if (process.env.AWSKMS_TEST_REAL === '1') {
    ...
    console.error('  AWSKMS_MODULE=build-aws/awskms.dylib');
    process.exit(2);
  }
...
/*
 * The end-to-end suites need a node that can pass a URL to createPrivateKey --
 * the OSSL_STORE loader support from nodejs/node's keyobject-stores work, which is
 * not in a released node yet. Detect it rather than let every test fail with an
 * opaque ERR_INVALID_ARG_TYPE, and say plainly what is missing.
 *
...
const probe = spawn(nodeBin, ['-e', `
  ...
  try {
    createPrivateKey({ key: new URL('awskms:key-id=probe') });
  } catch (err) {
...
if (!hasStoreLoader) {
  ...
  console.log('# Skipping the end-to-end suites: they need the OSSL_STORE loader');
  console.log('# support from nodejs/node, which is not in a released node yet.');
  console.log('#');
  ...
  console.log('#');
  console.log('# Point AWSKMS_NODE at a node built from that branch to run these.');
  await stub?.close();
  process.exit(0);
}
