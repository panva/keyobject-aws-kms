...
const faulty = (marker, spec) =>
  createPrivateKey({ key: new URL(`awskms:key-id=alias/${marker}-${spec}`) });
...
describe('KMS exceptions map to distinct error codes', { skip: needsHttpStub }, () => {
  const MAPPINGS = [
    ...
    ['KMSInvalidStateException', 'ERR_OSSL_AWSKMS_KEY_PENDING_DELETION'],
  ...
  for (const [exception, code] of MAPPINGS) {
    test(`${exception} at load -> ${code}`, () => {
      assert.throws(
        () => createPrivateKey({ key: new URL(`awskms:key-id=alias/fault-err-${exception}-RSA_2048`) }),
        { code, library: 'awskms' },
      );
    });
  }
  ...
  for (const [exception, code] of [
    ...
    test(`${exception} at sign time -> ${code}`, () => {
      const key = createPrivateKey({
        key: new URL(`awskms:key-id=alias/fault-signerr-${exception}-RSA_2048`),
      });
  ...
  test('an unmapped exception at sign time yields SIGN_FAILED, not the load code', () => {
    const key = createPrivateKey({
      key: new URL('awskms:key-id=alias/fault-signerr-KeyUnavailableException-RSA_2048'),
    });
...
describe('a key that is not for signing', { skip: needsHttpStub }, () => {
  test('an ENCRYPT_DECRYPT key is refused at load, before any signing is attempted', () => {
    ...
    assert.throws(
      () => createPrivateKey({ key: new URL('awskms:key-id=alias/fault-keyusage-RSA_2048') }),
      { code: 'ERR_OSSL_AWSKMS_INVALID_KEY_USAGE', library: 'awskms' },
    );
  });
});
...
describe('throttling is retried before it is reported', { skip: needsHttpStub }, () => {
  ...
  test('a throttled sign retries, then reports THROTTLED', () => {
    const key = createPrivateKey({
      key: new URL('awskms:key-id=alias/fault-signerr-ThrottlingException-RSA_2048'),
    });
  ...
  for (const [name, padding] of [
    ...
    test(`${name} is refused`, () => {
      assert.throws(
        ...
        { code: 'ERR_OSSL_AWSKMS_UNSUPPORTED_PADDING', library: 'awskms' },
      );
    });
  }
});
