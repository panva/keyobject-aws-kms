...
void test_spki(void) {
  ...
  for (const AWSKMS_KEYSPEC *spec = awskms_keyspec_all();
    ...
    /* A spec/key-type mismatch must be caught, not guessed around. */
    {
      ...
      CHECK(awskms_pubkey_from_spki(&bad, der, (size_t)der_len, other, NULL,
                                    NULL) == 0,
  ...
  /* Unknown and malformed inputs. */
  {
    ...
    memset(&pub, 0, sizeof(pub));
    CHECK(awskms_pubkey_from_spki(&pub, junk, sizeof(junk), "RSA_2048", NULL,
                                  NULL) == 0,
          "junk DER must be rejected");
    awskms_pubkey_cleanup(&pub);

    memset(&pub, 0, sizeof(pub));
    CHECK(awskms_pubkey_from_spki(&pub, junk, sizeof(junk), "SM2", NULL,
                                  NULL) == 0,
          "SM2 is out of scope and must be rejected");
    awskms_pubkey_cleanup(&pub);

    memset(&pub, 0, sizeof(pub));
    CHECK(awskms_pubkey_from_spki(&pub, NULL, 0, "RSA_2048", NULL, NULL) == 0,
...
void test_keyspec(void) {
  ...
  const AWSKMS_KEYSPEC *k256 = awskms_keyspec_by_name("ECC_SECG_P256K1");
  ...
  CHECK(rsa != NULL && p256 != NULL && p384 != NULL && k256 != NULL &&
            ed != NULL && ml != NULL,
  ...
  CHECK_STR(k256->group_name, "secp256k1", "secp256k1 group name");
  ...
  CHECK_STR(awskms_signing_algorithm(k256, AWSKMS_DIGEST_SHA256, 0),
            "ECDSA_SHA_256", "secp256k1 sha256");
